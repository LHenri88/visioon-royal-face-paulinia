...
export function SectionHead({ kicker, title, sub }: { kicker?: string; title: string; sub?: string }) {
  return (
    <div className="reveal max-w-2xl mx-auto text-center mb-14">
      {kicker && <span className="inline-block text-xs font-semibold tracking-widest uppercase mb-2" style={{ color: 'var(--color-primary)' }}>{kicker}</span>}
      {kicker && <span className="kicker-bar" />}
